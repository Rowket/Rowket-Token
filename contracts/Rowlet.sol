...
//                                                 
// TG: https://t.me/rowletfinance
// Web: https://rowlet.finance
...
contract RowletToken is Getters, Setters, Context, IBEP20, Ownable {
    ...
    string  private constant _NAME = 'Rowlet.finance';
    string  private constant _SYMBOL = 'ROW';
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_isSupportedPool(sender) || _isSupportedPool(recipient)) {
            isTaxFree = false;
        } else {
            isTaxFree = true;
        }
    ...
    function _setBurnFee(uint256 burnFee) external onlyOwner() {
        require(taxFee >= 0 && taxFee <= 100, 'Fee has to be between 0-10');
        _BURN_FEE = burnFee;
    }
    ...
    function _isSupportedPool(address pool) private view returns(bool){
        return isSupportedPool(pool);
    }
    
    /*
      @dev add or remove supported pools for which utility fees do not apply
      provides Token utility for any DEX that lists ROW, beyond PancakeSwap
    */

     function _addNewSupportedPool(address pool) external onlyOwner() {
        addSupportedPool(pool);
    }

    function _removeOldSupportedPool(address pool) external onlyOwner() {
